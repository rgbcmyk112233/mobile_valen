...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  String _getOriginLocation() {
    // Ambil lokasi unik, filter yang null/kosong
    final locations = widget.items
        .map((e) => e.lokasi ?? "Jakarta") // Default Jakarta jika null
        .toSet()
        .toList();

    if (locations.isEmpty) return "Jakarta"; // Fallback aman

    if (locations.length == 1) {
      return locations.first; // ✅ Tampilkan string murni dari data
    } else {
      return "Berbagai Gudang"; // Jika barang berasal dari banyak tempat
    }
  }
  ...
  void _updateShippingLogic() {
    ...
    setState(() {
      ...
      // B. Logika Estimasi Hari (Cek Kesamaan String)
      // Kita cek apakah "Nama Negara Tujuan" ada di dalam string "Asal Barang"
      bool isDomestic = asalBarang.toLowerCase().contains(negara.toLowerCase());

      // Jika asal barang tidak mengandung nama negara tujuan, dianggap Internasional
      // (Kecuali default "Jakarta" dianggap Indonesia)
      if (negara == "Indonesia" &&
      ...
      if (isDomestic) {
        ...
        if (sameCity) {
          estimasiHariMin = 1;
          estimasiHariMax = 2; // Satu Kota
        } else {
          estimasiHariMin = 2;
          estimasiHariMax = 4; // Beda Kota

          // Case khusus pulau jauh di Indonesia
          if (negara == "Indonesia" &&
  ...
  // Label di atas Input (Agar tidak tumpang tindih)
